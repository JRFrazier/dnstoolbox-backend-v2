...
class TcpCheck extends RESTDataSource {
    constructor() {
        super();
        this.baseURL = 'https://api.sonar.constellix.com/rest/checks/';
    }
    willSendRequest(request) {
        request.headers.set('x-cns-security-token', token());
    }

    async tcpCheck(location, hostname, ipVersion, port) {
        const response = await this.post(`test/${location}`,
            {
                "protocolTypes": `TCP`, "interval": "ONEMINUTE", "ipversion": `${ipVersion}`, "port": `${port}`, "ip": `${hostname}`, "expectedStatusCode": "200"
            }
        )
        return response
    }
